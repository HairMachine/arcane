function distiller_output_entity_create(gm as Gamestate, x as integer, y as integer) as integer
	...
	gm.cl.add(new TagComponent(out_eid, "DistillerOutputComponent"))
	return out_eid
end function
...
function distiller_input_entity_create(gm as Gamestate, x as integer, y as integer) as integer
	...
	gm.cl.add(new TagComponent(inp_eid, "DistillerInputComponent"))
	return inp_eid
end function
...
function distiller_switch_entity_create(gm as Gamestate, x as integer, y as integer) as integer
	...
	gm.cl.add(new TagComponent(eid, "DistillerSwitchComponent"))
	return eid
end function

sub DistillerUseSystem(user as integer, gm as Gamestate)
	dim s as TagComponent ptr = cast(TagComponent ptr, gm.cl.filter("DistillerSwitchComponent").components(0))
	...
	if switch_pos->x = user_pos->x and switch_pos->y = user_pos->y then
		...
		dim in as TagComponent ptr = cast(TagComponent ptr, gm.cl.filter("DistillerInputComponent").components(0))
		...
		if in_count = 2 then
			...
			next
			'create an elixir
			dim o as TagComponent ptr = cast(TagComponent ptr, gm.cl.filter("DistillerOutputComponent").components(0))
